...
CREATE TABLE `staff` (
         `gid` BIGINT UNSIGNED AUTO_INCREMENT COMMENT '唯一id',
...
DROP TABLE IF EXISTS `staff_oauth_client_details`;
CREATE TABLE `oauth_client_details_staff` (
    `client_id` VARCHAR(255) COMMENT '客户端id',
    `staff_id` BIGINT UNSIGNED COMMENT '员工id',
    CONSTRAINT client_id_cons FOREIGN KEY (client_id) REFERENCES oauth_client_details(client_id),
    CONSTRAINT staff_id_cons FOREIGN KEY (staff_id) REFERENCES staff(gid)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
DESC oauth_client_details_staff;
